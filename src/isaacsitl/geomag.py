"""World Magnetic Model lookup -- the synthetic magnetometer's Earth field.

EKF2 won't fuse the magnetometer (so yaw never aligns, the craft never arms) unless the
measured field's strength + inclination match its INTERNAL WMM at the GPS location
(`Ekf::checkMagField`). So the HIL mag must be built from the same model, not a guess.
These are the WMM2018 grids + bilinear lookup from PX4-SITL_gazebo (the source PX4's own
`geo_mag_declination` tables derive from), as ported by Pegasus -- pure stdlib.

Grids are kept as whitespace strings (parsed at import) so the formatter can't explode them
into one-int-per-line. Each is 13 rows (lat -60..60) x 37 cols (lon -180..180), 10 deg step.
"""

# ruff: noqa: E501 -- the WMM grid string rows are intentionally wide data, not code.
...
def _grid(text: str) -> list[list[int]]:
    """Parse a whitespace table into rows of ints."""
...
# Total field strength [centi-Tesla]; * 0.01 -> gauss (1 gauss = 100 microTesla).
_STRENGTH = _grid("""
...
_RES = 10.0
_MIN_LAT, _MAX_LAT = -60.0, 60.0
_MIN_LON, _MAX_LON = -180.0, 180.0


def _index(val: float, lo: float, hi: float) -> int:
    """Clamp to the grid and return the SW lookup index (room for index+1)."""
    val = min(max(val, lo), hi - _RES)
    return int((-lo + val) / _RES)


def _lookup(lat: float, lon: float, table: list[list[int]]) -> float:
    """Bilinear interpolation of a WMM grid at (lat, lon) [deg]."""
    if lat < -90.0 or lat > 90.0 or lon < -180.0 or lon > 180.0:
        return 0.0
    min_lat = int(lat / _RES) * _RES
    min_lon = int(lon / _RES) * _RES
    i = _index(min_lat, _MIN_LAT, _MAX_LAT)
    j = _index(min_lon, _MIN_LON, _MAX_LON)
    sw, se = table[i][j], table[i][j + 1]
    ne, nw = table[i + 1][j + 1], table[i + 1][j]
    lat_s = min(max((lat - min_lat) / _RES, 0.0), 1.0)
    lon_s = min(max((lon - min_lon) / _RES, 0.0), 1.0)
    lo = lon_s * (se - sw) + sw
    hi = lon_s * (ne - nw) + nw
    return lat_s * (hi - lo) + lo


def declination_deg(lat: float, lon: float) -> float:
    """WMM magnetic declination [deg] at (lat, lon)."""
    return _lookup(lat, lon, _DECLINATION)


def inclination_deg(lat: float, lon: float) -> float:
    """WMM magnetic inclination (dip) [deg] at (lat, lon)."""
    return _lookup(lat, lon, _INCLINATION)


def strength_gauss(lat: float, lon: float) -> float:
    """WMM total field strength [gauss] at (lat, lon)."""
    return 0.01 * _lookup(lat, lon, _STRENGTH)


def field_ned_gauss(lat: float, lon: float) -> tuple[float, float, float]:
    """Earth magnetic field (north, east, down) [gauss] at (lat, lon).

    Standard decomposition: H = F.cos(I) horizontal, declination D rotates it in the N/E
    plane, inclination I tips it down.
    """
    decl = math.radians(declination_deg(lat, lon))
    incl = math.radians(inclination_deg(lat, lon))
    f = strength_gauss(lat, lon)
    h = f * math.cos(incl)
    return h * math.cos(decl), h * math.sin(decl), f * math.sin(incl)
